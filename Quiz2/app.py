...
def displaydata():
    ...
    print(stateAbb)
    sql = "SELECT candidate,party_detailed,totalvotes FROM election WHERE year = ? AND state_po LIKE ?"
    ...
    #     sql = "SELECT * FROM election WHERE year > ? AND candidatevotes > ?"
    #     values = [year1,candVote]
    #     cursor.execute(sql,values)
    # end = time.time()
    # executiontime = end - start
    return render_template('people.html', rows=rows)

@app.route('/rangedata', methods=['GET','POST'])
def rangedata():
    minVote = request.form['minvote']
    maxVote = request.form['maxvote']
    minyear = request.form['minyear']
    maxyear = request.form['maxyear']
    # year1 = str(request.form.get('year1'))
    print(minVote)
    # stateAbb = str(request.form.get('state1'))
    # print(stateAbb)
    sql = "SELECT candidate, party_detailed, candidatevotes FROM election WHERE candidatevotes BETWEEN ? AND ? AND year BETWEEN ? and ?"

    values = [minVote,maxVote,minyear,maxyear]
    cursor.execute(sql,values)
...
def candname():
    ...
    print(candname)
    start = time.time()
    sql = "SELECT year,candidatevotes FROM election WHERE candidate LIKE ?"
    values = [candname]
    cursor.execute(sql,values)
...
@app.route('/rangedatatime', methods=['GET','POST'])
def rangedatatime():
    minVote = request.form['minvote']
    maxVote = request.form['maxvote']
    minyear = request.form['minyear']
...
@app.route('/rangedatatimecache', methods=['GET','POST'])
def rangedatatimecache():
    minVote = request.form['minvote']
    maxVote = request.form['maxvote']
    minyear = request.form['minyear']
    maxyear = request.form['maxyear']
